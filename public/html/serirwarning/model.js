...
const colors = {
  SUSCEPTIBLE: 'rgba(65,115,239,0.78)',
  ASYMPTOMATIC: 'rgba(201,205,85,0.78)',
  INFECTED: 'rgba(224,13,69,0.78)',
  REMOVED: 'rgba(45,184,57,0.78)'
}
...
class Person {
  constructor(type) {
    ...
    if (Math.random() < socialDistancingRate) { this.practicesSocialDistance = true } else { this.practicesSocialDistance = false }
    if (type === types.SUSCEPTIBLE) { this.color = colors.SUSCEPTIBLE } else if (type === types.INFECTED) { this.color = colors.INFECTED } else if (type === types.ASYMPTOMATIC) { this.color = colors.ASYMPTOMATIC } else if (type === types.REMOVED) { this.color = colors.REMOVED }
  }

  draw() {
    context.beginPath()
    context.arc(this.cx, this.cy, this.radius, 0.0, 2 * Math.PI, false)
    ...
    context.stroke()
    context.beginPath()
    context.arc(this.cx, this.cy, infectionCircleRadius, 0.0, 2 * Math.PI, false)
    context.strokeStyle = 'rgba(220,220,220,0)'
  ...
  move() {
    ...
    if (this.cx > width - 2 * this.radius || this.cx < 2 * this.radius) {
      this.speed_x *= -1
      if (this.cx > width - 2 * this.radius) { this.cx = width - 2 * this.radius } else if (this.cx < 2 * this.radius) { this.cx = 2 * this.radius }
    }

    if (this.cy > height - 2 * this.radius || this.cy < 2 * this.radius) {
      this.speed_y *= -1
      if (this.cy > height - 2 * this.radius) { this.cy = height - 2 * this.radius } else if (this.cy < 2 * this.radius) { this.cy = 2 * this.radius }
    }

    if (Math.abs(this.speed_x) > this.max_speed) { this.speed_x = Math.sign(this.speed_x) * this.max_speed }

    if (Math.abs(this.speed_y) > this.max_speed) { this.speed_y = Math.sign(this.speed_y) * this.max_speed }
...
var chart = new CanvasJS.Chart('chartContainer', {
  ...
  axisY: {
    ...
    labelFontColor: 'rgba(255,255,255,.7)',
...
var updateCounts = function() {
  susCountDiv.textContent = '易感者: ' + n_susceptible
  infCountDiv.textContent = '感染者: ' + n_infected
  remCountDiv.textContent = '康复者: ' + n_removed
  asympCountDiv.textContent = '潜伏者: ' + n_asymptomatic
}
// updateCounts();
// End -------------------------------------------
...
function NumShow() {
  document.getElementById('susceptibleCount').style.visibility = 'visible'
  document.getElementById('infectedCount').style.visibility = 'visible'
  document.getElementById('removedCount').style.visibility = 'visible'
  document.getElementById('asymptomaticCount').style.visibility = 'visible'
}
