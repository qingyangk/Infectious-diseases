...
export default {
  ...
  created() {
    var that = this
    that.activeName = that.spacetimedata.activeName
    that.name = that.spacetimedata.Bname
    if (that.name !== '') {
      setTimeout(function() {
        var e = ''
        if (that.name === '手足口病') {
  ...
  beforeDestroy() {
    eventBum.$off('typc')
    this.spacetimedata.activeName = this.activeName
    this.spacetimedata.Bname = this.name
  },
  mounted() {
    var that = this
    that.get()
