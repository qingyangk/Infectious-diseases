...
export default {
  ...
  created() {
    ...
    that.json.name = that.spacetimedata.name
    that.json.where = that.spacetimedata.where
  ...
  beforeDestroy() {
    this.spacetimedata.name = this.json.name
    this.spacetimedata.where = this.json.where
  },
  mounted() {
    var that = this
