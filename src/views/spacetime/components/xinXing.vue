<template>
  <div id="xxing" style="position: absolute;width: 100%;height: 100%">
...
export default {
  ...
  data() {
    return {
      ...
      // 新兴时空
      xinxing: undefined,
      url: 'http://114.98.239.36:6080/arcgis/rest/services/ShaanXiDisease/TimeSpaceHotSpots/MapServer/0',
      // view
      view: undefined,
      map: undefined,
      // gp服务
  ...
  beforeDestroy() {
    ...
    eventBum.$off('data')
    eventBum.$off('space')
    document.getElementById('xxing').style.display = 'none'
  },
  mounted() {
    // 城市尺度
  ...
  methods: {
    _Init: function() {
      ...
      loadModules(
        ...
        // 新兴图层
        that.xinxing = new FeatureLayer({
          url: this.url,
          title: '新兴时空热点分析'
        })
        that.xinxing.visible = false
        that.bxian.visible = false
        that.bShi.visible = false
        that.map = new Map({
          basemap: customBasemap,
          layers: [that.bSheng, that.bShi, that.bxian, that.xinxing]
        })
        ...
        that.view.on('drag', function name(params) {
          // console.log(that.view.center)
        })
        // 隐藏放大缩小
    ...
    areaQuery(sqlwhere, sqlwhereurl, e) {
      ...
      if (e === '制图') {
        if (that.json.timz === 'NaN-NaN-NaN') {
          ...
          that.xinxing.opacity = 0.9
          that.xinxing.visible = true
          that.view.whenLayerView(that.xinxing)
            .then(function(layerView) {
            })
          that.queryData.where = ''
          that.queryTask.url = that.url
