<template>
  <div id="right">
    <p style="color: #ffffff;margin: 10px 0;padding: 0;text-align: center;font-family: 'KuHei';font-weight: bold;">最新正面资讯TOP20</p>
    <i class="el-icon-refresh-right" style="position: absolute;color: #fff;top: 12px;right: 0px;font-size: 20px;cursor: pointer;" @click="chong()" />
    <ul>
    ...
    getData() {
      ...
      that.$store.dispatch('city/qLNews').then((dataZ) => {
        ...
        for (var i = 0; i < dataZ.length; i++) {
          that.gong = {}
          that.gong.zan = dataZ[i].fabulous * 1
        ...
        that.json = that.json.sort(function(a, b) {
          return b.zan - a.zan
        })
      })
    },
