<template>
  <div class="adddate">
    ...
    <div class="exhibition">
      <ul>
        <li>
          <p class="button1">放大</p>
          <p class="button2">缩小</p>
        </li>
      </ul>
    </div>
    <!-- 添加数据 -->
    ...
    <div id="hezi">
      <div class="upload">
        ...
        >
          <el-button ref="upload" size="mini" type="primary"
            >上传文件</el-button
          >
        </el-upload>
      </div>
    ...
    <div class="table">
      ...
      <div class="tool">
        <el-row :gutter="5">
          ...
          <el-col :span="2">
            <el-button type="primary" size="mini" :icon="icon" @click="clear()"
              >取消选择</el-button
            >
          </el-col>
          <el-col :span="16">
      ...
      <div class="body">
        ...
        >
          ...
          >
            <template v-slot:header>
              <span
                ><icon-svg icon-class="icon-danweiguanli" /> 患者工作单位</span
              >
            </template>
            <template v-slot:edit="scope">
          ...
          >
            <template v-slot:header>
              <span
                ><icon-svg icon-class="icon-shebeimingcheng" /> 病例分类</span
              >
            </template>
            <template v-slot:edit="scope">
          ...
          >
            <template v-slot:header>
              <span
                ><icon-svg icon-class="icon-zhongzhengjianhushi" />
                重症患者</span
              >
            </template>
            <template v-slot:edit="scope">
          ...
          >
            <template v-slot:header>
              <span
                ><icon-svg icon-class="icon-shiyanshianquan" /> 实验室结果</span
              >
            </template>
            <template v-slot:edit="scope">
...
<script>
import DataInsertion from "./components/dataInsertion"; // 数据插入
import Router from "./components/router"; // 侧边栏
    ...
    _Init: function () {
      ...
      loadModules(
        ...
        ([
          ...
          var url =
            "http://1.117.71.181:6080/arcgis/rest/services/cs/cspts/FeatureServer/0";
          ...
          view.when(function () {
            ...
            function editor(params) {
              ...
              for (let index = 0; index < that.tableData.length; index++) {
                ...
                that.tableData[index].发病日期 =
                  STime.getFullYear() +
                  "-" +
                  that.checkTime(STime.getMonth() + 1) +
                  "-" +
                  that.checkTime(STime.getDate());
                that.tableData[index].诊断时间 =
                  ETime.getFullYear() +
                  "-" +
                  that.checkTime(ETime.getMonth() + 1) +
                  "-" +
                  that.checkTime(ETime.getDate());
                that.tableData[index].发病日期 = String(
                  that.tableData[index].发病日期
                );
                that.tableData[index].诊断时间 = String(
                  that.tableData[index].诊断时间
                );
                const point = {
...
.adddate .table {
  width: 97.4%;
  height: 70px;
  background-color: rgba(31, 65, 120, 0.8);
