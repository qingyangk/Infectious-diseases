<template>
  <div id="jv" style="position: absolute;width: 100%;height: 100%">
  ...
  methods: {
    _Init() {
      ...
      loadModules(
        ...
        that.cluste.featureReduction = clusterConfig
        that.bxian.visible = false
        that.bShi.visible = false
        that.cluste.visible = false
        const map = new Map({
          basemap: customBasemap,
          layers: [that.bSheng, that.bShi, that.bxian, that.cluste]
        })
      ...
      if (e === '制图') {
        if (that.json.timz === 'NaN-NaN-NaN') {
          ...
          if (that.json.typh === '死亡') {
            that.cluste.definitionExpression = that.jbwhere + "疾病名称 = '" + that.jbtype + "' and 发病日期 > '" + that.json.timz + "' and 发病日期 < '" + that.json.timx + "'and 死亡日期 <> '.'"
          } else if (that.json.typh === '重症') {
            that.cluste.definitionExpression = that.jbwhere + "疾病名称 = '" + that.jbtype + "' and 发病日期 > '" + that.json.timz + "' and 发病日期 < '" + that.json.timx + "'and 重症患者 = '是'"
          } else if (that.json.typh === '诊断') {
            that.cluste.definitionExpression = that.jbwhere + "疾病名称 = '" + that.jbtype + "' and 发病日期 > '" + that.json.timz + "' and 发病日期 < '" + that.json.timx + "'"
          } else {
            that.cluste.definitionExpression = that.jbwhere + "疾病名称 = '" + that.jbtype + "' and 发病日期 > '" + that.json.timz + "' and 发病日期 < '" + that.json.timx + "'"
          }
          that.cluste.visible = true
        }
        setTimeout(() => {
          eventBum.$emit('gai', 1)
        }, 10000)
