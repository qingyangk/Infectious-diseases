<template>
  <div class="mrsw">
    <div id="mr" style="height: 100%;width: 100%;z-index: 1;" />
    <div id="zhegai">
...
export default {
  ...
  methods: {
    initCharts() {
      const that = this
      document.getElementById('mr').removeAttribute('_echarts_instance_')
      const myChart = echarts.init(document.getElementById('mr'))
      const option = {
        title: {
          text: '每日死亡'
        },
    ...
    getqAgestatistics() {
      ...
      if (that.region === 'yibaodao') {
        that.$store.dispatch('city/qDRDeath', that.getdata.name).then((dataH) => {
          that.CRDeathSF = []
          that.CRDeathSS = []
          that.CRDeathST = []
          that.vCRDeathSF = []
          ...
          that.rvCRDeathSS = []
          that.rvCRDeathST = []
          for (var i = 0; i < dataH.RRDeathSFTime.length; i++) {
            var time = new Date(dataH.RRDeathSFTime[i])
            dataH.RRDeathSFTime[i] = time.getFullYear() + '-' + that.checkTime(time.getMonth() + 1) + '-' + that.checkTime(time.getDate())
            that.alldateTime.push(dataH.RRDeathSFTime[i])
            if (that.updateTime === dataH.RRDeathSFTime[i - 1]) {
              that.vCRDeathSF.push(dataH.RRDeathSF[i - 1]) // 获取当前日期时间
              that.vCRDeathSS.push(dataH.RRDeathSS[i - 1])
              that.vCRDeathST.push(dataH.RRDeathST[i - 1])
              that.c = i
            }
            if (that.c) {
              that.vCRDeathSF.push(dataH.RRDeathSF[i]) // 获取预测数据
              that.vCRDeathSS.push(dataH.RRDeathSS[i])
              that.vCRDeathST.push(dataH.RRDeathST[i])
            } else {
              that.CRDeathSF.push(dataH.RRDeathSF[i]) // 获取历史数据
              that.vCRDeathSF.push('')
              that.CRDeathSS.push(dataH.RRDeathSS[i])
              that.vCRDeathSS.push('')
              that.CRDeathST.push(dataH.RRDeathST[i])
        ...
        that.$store.dispatch('city/qDRNDeath', that.getdata.name).then((dataH) => {
          that.CRDeathSF = []
          that.CRDeathSS = []
          that.CRDeathST = []
          that.vCRDeathSF = []
          ...
          that.rvCRDeathSS = []
          that.rvCRDeathST = []
          for (var i = 0; i < dataH.RADeathSFTime.length; i++) {
            var time = new Date(dataH.RADeathSFTime[i])
            dataH.RADeathSFTime[i] = time.getFullYear() + '-' + that.checkTime(time.getMonth() + 1) + '-' + that.checkTime(time.getDate())
            that.alldateTime.push(dataH.RADeathSFTime[i])
            if (that.updateTime === dataH.RADeathSFTime[i - 1]) {
              that.vCRDeathSF.push(dataH.RADeathSF[i - 1]) // 获取当前日期时间
              that.vCRDeathSS.push(dataH.RADeathSS[i - 1])
              that.vCRDeathST.push(dataH.RADeathST[i - 1])
              that.c = i
            }
            if (that.c) {
              that.vCRDeathSF.push(dataH.RADeathSF[i]) // 获取预测数据
              that.vCRDeathSS.push(dataH.RADeathSS[i])
              that.vCRDeathST.push(dataH.RADeathST[i])
            } else {
              that.CRDeathSF.push(dataH.RADeathSF[i]) // 获取历史数据
              that.vCRDeathSF.push('')
              that.CRDeathSS.push(dataH.RADeathSS[i])
              that.vCRDeathSS.push('')
              that.CRDeathST.push(dataH.RADeathST[i])
              that.vCRDeathST.push('')
            }
          }
          if (that.tree.length === 1) {
