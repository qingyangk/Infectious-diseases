<template>
  <div class="regionalranking">
    <div id="regionalranking" style="height: 100%;width: 100%;z-index: 99;" />
  </div>
</template>
<script>
...
export default {
  ...
  mounted() {
    eventBus.$on('pushMsg', (children1Msg) => {
      ...
      document.getElementById('regionalranking').remove()
      const div = document.createElement('div')
      div.id = 'regionalranking'
      // div.style.position = 'absolute'
      div.style.height = '100%'
      div.style.width = '100%'
      div.style.zIndex = '99'
      const maps = document.getElementsByClassName('regionalranking')[0]
      maps.append(div)
      this.getData()
    })
    var that = this
  ...
  methods: {
    initCharts() {
      const that = this
      const myChart = echarts.init(document.getElementById('regionalranking'))
      const option = {
        color: ['rgb(242, 75, 61)', 'rgb(150, 150, 150)'],
        title: {
          left: 'left',
          text: that.data.name + ' 地区排名',
          textStyle: {
            fontSize: 15,
      ...
      window.addEventListener('resize', function() {
        that.chart = echarts.init(document.getElementById('regionalranking'))
        that.chart.resize()
      })
    ...
    getData() {
      ...
      that.$store.dispatch('city/qRanking', this.data).then((dataZ) => {
