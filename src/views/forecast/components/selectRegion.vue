...
export default {
  ...
  created() {
    this.citys = JSON.parse(window.localStorage.getItem('city')) // 读取值
  },
  mounted() {
    var that = this
  ...
  methods: {
    async jihuo() {
      ...
      // 样式
      if (path === 'seir') {
        city.style.right = '425px'
      } else if (path === 'CForecast') {
        city.style.right = '410px'
      } else if (path === 'arima') {
        city.style.right = '410px'
        that.json.where = 0
        that.json.name = '中国'
      } else {
        city.style.right = '5px'
      }
    },
    // 弹出弹出层
    tan() {
...
#selectRegion .city li:nth-child(1) {
  ...
  width: 25px;
  font-size: 15px;
  border-right: 1.5px solid #0fafeb;
  cursor: pointer;
}
...
#selectRegion .city li:nth-child(3) {
  ...
  width: 35px;
  font-size: 18px;
  border-left: 1.5px solid #0fafeb;
}
/* 弹出层 */
#selectRegion #choice {
  opacity: 0;
