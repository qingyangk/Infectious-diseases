...
export default {
  ...
  methods: {
    ...
    int() {
      ...
      this.yCon = this.changeTime(JSON.parse(JSON.stringify(this.typeName(concount))))
      this.yCure = this.changeTime(JSON.parse(JSON.stringify(this.typeName(curedCount))))
      this.yDead = this.changeTime(JSON.parse(JSON.stringify(this.typeName(deadCount))))
    },
    // 排序函数
    typeName(data) {
      // var that = this
    ...
    changeTime(datas) {
      for (let index = 0; index < datas.length; index++) {
        ...
        for (let i = 0; i < element1.length; i++) {
          element1[i][1] = element1[i][1].substring(0, 4) + '-' + element1[i][1].substring(4, 6) + '-' + element1[i][1].substring(6, 8)
        }
      }
      return datas
    },
    ...
    timecovid() {
      ...
      for (let j = 0; j < 5; j++) {
        ...
        dataAll.push({ name: element.Province, type: 'line', lineStyle: { width: 4 }, data: dat })
      }
    ...
    initCharts(data, deaddata, curedata, name) {
      ...
      // eslint-disable-next-line no-undef
      var option = {
        color: ['rgb(255, 121, 121)', 'rgb(194, 196, 89)', '#3A84FF'],
        grid: {
          top: '12%',
          left: '2%',
          right: '6%',
        ...
        tooltip: {
          ...
          formatter: function(params) {
            console.log(params)
            var rez = that.json.name + '预测<br/>' +
              params[2].marker + '确诊数据' + '：' + '<span style="font-weight: bold">' + params[0].data + '</span><br/>' +
              params[1].marker + '治愈数据' + '：' + '<span style="font-weight: bold">' + params[2].data + '</span><br/>' +
              params[0].marker + '死亡数据' + '：' + '<span style="font-weight: bold">' + params[1].data + '</span>'
            return rez
          },
          textStyle: {
            fontWeight: 'bold',
    ...
    start() {
      ...
      var mag2 = ['all', ['>=', ['get', that.xName[that.subscript]], 200], ['<', ['get', that.xName[that.subscript]], 400]]
      var mag3 = ['all', ['>=', ['get', that.xName[that.subscript]], 400], ['<', ['get', that.xName[that.subscript]], 600]]
      var mag4 = ['all', ['>=', ['get', that.xName[that.subscript]], 600], ['<', ['get', that.xName[that.subscript]], 800]]
      var mag5 = ['all', ['>=', ['get', that.xName[that.subscript]], 800], ['<', ['get', that.xName[that.subscript]], 1000]]
      var mag6 = ['>=', ['get', that.xName[that.subscript]], 2000]
      var colors = ['#80ffdb', '#64dfdf', '#48bfe3', '#5390d9', '#6930c3', '#ff7979']
      if (this.json.name === '中国') {
        this.map.setPaintProperty('maine0', 'fill-color', ['case',
        // 定义不同层次数据颜色
          mag1, colors[0],
          mag2, colors[1],
          mag3, colors[2],
