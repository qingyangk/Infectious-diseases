<template>
  <div id="numberCasess">
    <div id="numberCases" style="height: 100%;width: 100%;z-index: 1;" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    initCharts(names, datas) {
      var that = this
      this.myChart = echarts.init(document.getElementById('numberCases'))
      const option = {
        // 标题
        title: {
          text: that.json.time  + that.json.name + '-' + that.json.type + '-' + '发病数',
          left: 'center',
          padding: 10,
          textStyle: {
      ...
      window.addEventListener('resize', function() {
        var chart = echarts.init(document.getElementById('numberCases'))
        if (chart) {
          chart.resize() // 不报错
        }
      })
    ...
    getJson() {
      var that = this
      that.$store.dispatch('ndatapanel/QNumberCases', that.json).then((dataz) => {
    ...
    CAdestroy() {
      ...
      // document.getElementById('numberCases').remove()
      ...
      // div.style.width = '100%'
      // div.style.zIndex = '1'
      // const numberCasess = document.getElementById('numberCasess')
      // numberCasess.append(div)
    }
  }
}
</script>
