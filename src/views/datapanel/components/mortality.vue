<template>
  <div id="mortalitys">
    <div id="mortality" style="height: 100%;width: 100%;z-index: 1;" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    initCharts(names, datas) {
      var that = this
      this.myChart = echarts.init(document.getElementById('mortality'))
      const option = {
        // 标题
        title: {
          text: that.json.time  + that.json.name + '-' + that.json.type + '-' + '死亡率',
          left: 'center',
          padding: 10,
          textStyle: {
      ...
      window.addEventListener('resize', function() {
        var chart = echarts.init(document.getElementById('mortality'))
        if (chart) {
          chart.resize() // 不报错
        }
      })
    ...
    getJson() {
      var that = this
      that.$store.dispatch('ndatapanel/QMortality', that.json).then((dataz) => {
        if (that.json.detailed === 0) {
          ...
          for (var i = 0; i < dataz.length; i++) {
            ...
            datas.push(dataz[i].死亡率.toFixed(2))
          }
          ...
          for (var k = 0; k < dataz.length; k++) {
            ...
            datae.push(dataz[k].死亡率.toFixed(2))
          }
    ...
    CAdestroy() {
      ...
      // document.getElementById('mortality').remove()
      ...
      // div.style.width = '100%'
      // div.style.zIndex = '1'
      // const mortalitys = document.getElementById('mortalitys')
      // mortalitys.append(div)
    }
  }
}
</script>
