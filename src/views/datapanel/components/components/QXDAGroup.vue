<template>
  <div class="QXDAGroup">
    <div id="QXDAGroup" style="height: 100%;width: 100%;z-index: 1;" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    initCharts(data) {
      const that = this
      this.myChart = echarts.init(document.getElementById('QXDAGroup'))
      const option = {
        // color:['#694EEB','#ff0000','#91CC75','#9c5333','#3B8E72','#3B8E72'],
        title: {
          text: that.data.time + '-' + this.data.name + '-诊断-年龄组分布',
          left: 'center',
          padding: 10,
          textStyle: {
      ...
      window.addEventListener('resize', function() {
        that.chart = echarts.init(document.getElementById('QXDAGroup'))
        that.chart.resize()
      })
    ...
    CAdestroy() {
      ...
      // document.getElementById('QXDAGroup').remove()
      ...
      // div.style.width = '100%'
      // div.style.zIndex = '1'
      // const QXDAGroup = document.getElementsByClassName('QXDAGroup')[0]
      // QXDAGroup.append(div)
    }
  }
}
</script>
