<template>
  <div class="QXDVType">
    <div id="QXDVType" style="height: 100%;width: 100%;z-index: 1;" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    initCharts(data) {
      const that = this
      this.myChart = echarts.init(document.getElementById('QXDVType'))
      // 图例
      var legend = []
      ...
      const option = {
        title: {
          text: that.data.time  + this.data.name + '-诊断-病毒类型',
          left: 'center',
          padding: 10,
          textStyle: {
      ...
      window.addEventListener('resize', function() {
        that.chart = echarts.init(document.getElementById('QXDVType'))
        that.chart.resize()
      })
    ...
    CAdestroy() {
      ...
      // document.getElementById('QXDVType').remove()
      ...
      // div.style.width = '100%'
      // div.style.zIndex = '1'
      // const QXDVType = document.getElementsByClassName('QXDVType')[0]
      // QXDVType.append(div)
    },
    color() {
