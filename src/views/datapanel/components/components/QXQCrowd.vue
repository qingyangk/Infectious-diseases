<template>
  <div class="QXQCrowd">
    <div id="QXQCrowd" style="height: 100%;width: 100%;z-index: 1;" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    initCharts(data) {
      const that = this
      this.myChart = echarts.init(document.getElementById('QXQCrowd'))
      const option = {
        title: {
          text: this.data.name + '-死亡-人群分布',
          left: 'center',
          padding: 10,
          textStyle: {
      ...
      window.addEventListener('resize', function() {
        that.chart = echarts.init(document.getElementById('QXQCrowd'))
        that.chart.resize()
      })
    ...
    CAdestroy() {
      ...
      // document.getElementById('QXQCrowd').remove()
      ...
      // div.style.width = '100%'
      // div.style.zIndex = '1'
      // const QXQCrowd = document.getElementsByClassName('QXQCrowd')[0]
      // QXQCrowd.append(div)
    }
  }
}
