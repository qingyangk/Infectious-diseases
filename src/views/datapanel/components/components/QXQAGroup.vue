<template>
  <div class="QXQAGroup">
    <div id="QXQAGroup" style="height: 100%;width: 100%;z-index: 1;" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    initCharts(data) {
      const that = this
      this.myChart = echarts.init(document.getElementById('QXQAGroup'))
      const option = {
        // color:['#694EEB','#ff0000','#91CC75','#9c5333','#3B8E72','#3B8E72'],
        title: {
          text: this.data.name + '-死亡-年龄组分布',
          left: 'center',
          padding: 10,
          textStyle: {
      ...
      window.addEventListener('resize', function() {
        that.chart = echarts.init(document.getElementById('QXQAGroup'))
        that.chart.resize()
      })
    ...
    CAdestroy() {
      ...
      // document.getElementById('QXQAGroup').remove()
      ...
      // div.style.width = '100%'
      // div.style.zIndex = '1'
      // const QXQAGroup = document.getElementsByClassName('QXQAGroup')[0]
      // QXQAGroup.append(div)
    }
  }
}
</script>
