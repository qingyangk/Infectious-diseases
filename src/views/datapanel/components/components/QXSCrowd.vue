<template>
  <div class="QXSCrowd">
    <div id="QXSCrowd" style="height: 100%;width: 100%;z-index: 1;" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    initCharts(data) {
      const that = this
      this.myChart = echarts.init(document.getElementById('QXSCrowd'))
      const option = {
        title: {
          text: that.data.time + '-' + this.data.name + '-重症-人群分布',
          left: 'center',
          padding: 10,
          textStyle: {
      ...
      window.addEventListener('resize', function() {
        that.chart = echarts.init(document.getElementById('QXSCrowd'))
        that.chart.resize()
      })
    ...
    CAdestroy() {
      ...
      // document.getElementById('QXSCrowd').remove()
      ...
      // div.style.width = '100%'
      // div.style.zIndex = '1'
      // const QXSCrowd = document.getElementsByClassName('QXSCrowd')[0]
      // QXSCrowd.append(div)
    }
  }
}
