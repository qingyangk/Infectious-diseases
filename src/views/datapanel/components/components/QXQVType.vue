<template>
  <div class="QXQVType">
    <div id="QXQVType" style="height: 100%;width: 100%;z-index: 1;" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    initCharts(data) {
      const that = this
      this.myChart = echarts.init(document.getElementById('QXQVType'))
      // 图例
      var legend = []
      ...
      window.addEventListener('resize', function() {
        that.chart = echarts.init(document.getElementById('QXQVType'))
        that.chart.resize()
      })
    ...
    CAdestroy() {
      ...
      // document.getElementById('QXQVType').remove()
      ...
      // div.style.width = '100%'
      // div.style.zIndex = '1'
      // const QXQVType = document.getElementsByClassName('QXQVType')[0]
      // QXQVType.append(div)
    },
    color() {
