<template>
  <div id="numberDeathss">
    <div id="numberDeaths" style="height: 100%;width: 100%;z-index: 1;" />
  </div>
</template>
<script>
...
export default {
  ...
  methods: {
    ...
    initCharts(names, datas) {
      var that = this
      this.myChart = echarts.init(document.getElementById('numberDeaths'))
      const option = {
        // 标题
        title: {
          text: that.json.time + that.json.name + '-' + that.json.type + '-' + '死亡数',
          left: 'center',
          padding: 10,
          textStyle: {
      ...
      window.addEventListener('resize', function() {
        var chart = echarts.init(document.getElementById('numberDeaths'))
        if (chart) {
          chart.resize() // 不报错
        }
      })
    ...
    getJson() {
      var that = this
      that.$store.dispatch('ndatapanel/QNumberDeaths', that.json).then((dataz) => {
    ...
    CAdestroy() {
      ...
      // document.getElementById('numberDeaths').remove()
      ...
      // div.style.width = '100%'
      // div.style.zIndex = '1'
      // const numberDeathss = document.getElementById('numberDeathss')
      // numberDeathss.append(div)
    }
  }
}
</script>
